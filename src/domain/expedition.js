import { ITEMS, QUALITY_ORDER } from '../data/items.js?v=3.0.0';
import { CONTAINER_TYPES } from '../data/containers.js?v=3.0.0';
import { REGIONS, REGION_BY_ID, CURIOS, CONTRACTS } from '../data/expedition.js?v=3.0.0';
import { CONNECTIONS, START_LOCATION_ID } from '../data/map.js?v=3.0.0';
import { EQUIPMENT, MEDICINES } from '../data/equipment.js?v=3.0.0';
import { ENCOUNTERS } from '../data/encounters.js?v=3.0.0';
import { fits, firstFit, usedArea, organize } from './inventory.js?v=3.0.0';

export const SAVE_KEY = 'mini-delta-force-expedition-v3';
export const RAID_SECONDS = 30 * 60;
export const SEARCH_SECONDS = 45;
export const EXTRACT_SECONDS = 60;
const LEGACY_CURIOS = [
...
export function newState() {
  return {version:3,money:50000,owned:{...Object.fromEntries(Object.keys(EQUIPMENT).map(id=>[id,0])),small:1,universal:1},equipment:{bag:'small',rig:'universal'},
    medicines:{'consumable-14020000003':2,medkit:1,bandage:2,'field-med':0},
    loadout:{'consumable-14020000003':2,medkit:0,bandage:1,'field-med':0},
    collection:{},eventBook:{},stats:{runs:0,successes:0,boxes:0,best:0},run:null,lastResult:null};
}
...
function createSpaces(equipment) {
  const spaces=[];
...
export function nearestExtraction(run){
  ...
  while(queue.length){queue.sort((a,b)=>a.seconds-b.seconds);const node=queue.shift();if(seen.has(node.id))continue;seen.add(node.id);
    if(REGION_BY_ID[node.id].extract)return node;
...
function settle(state,success,reason){
  ...
  for(const item of items.filter(e=>MEDICINES[e.itemId]))state.medicines[item.itemId]=(state.medicines[item.itemId]||0)+1;
...
export function act(previous,action){
  ...
  if(['buyGear','equip','buyMed','loadMed'].includes(action.type)){
    ...
    if(action.type==='buyGear'||action.type==='equip'){
      ...
      if(!state.owned[gear.id])fail('请先购买装备。');state.equipment[gear.kind]=gear.id;
    }else{
      const med=MEDICINES[action.id];if(!med)fail('没有这种药品。');
      if(action.type==='buyMed'){if(state.money<med.price)fail('资金不足。');state.money-=med.price;state.medicines[action.id]++;}
      else{const n=action.count;if(!Number.isInteger(n)||n<0||n>state.medicines[action.id]||n>12)fail('携带数量不可用。');state.loadout[action.id]=n;}
    }
    return state;
  }
  if(action.type==='start'){
    ...
    for(const id of Object.keys(MEDICINES)){const count=Math.min(state.loadout[id]||0,state.medicines[id]||0);for(let i=0;i<count;i++){if(!put(run,entry(run,id,true)))fail('选择的药品放不下，请减少携带数量或购买更大的装备。');state.medicines[id]--;}}
  ...
  switch(action.type){
    ...
    case 'useMed':{
      ...
      if(!run.bleeding&&(!med.heal||run.hp===100))fail('当前状态无需使用这件药品。');
      const cost=med.seconds+(['bag','safeBox'].includes(source.kind)?5:0);source.items=source.items.filter(e=>e.id!==item.id);
      if(!spend(run,cost))break;
      effects(run,{heal:med.heal,stopBleed:med.stopBleed});log(run,`使用${med.name}，耗时 ${cost} 秒，当前生命 ${run.hp}/100。`);break;
    }
    case 'closeLoot':{
    ...
    case 'extract':{
      requireFree(run);if(!REGION_BY_ID[run.locationId].extract)fail('需要先抵达东部或西部撤离点。');
      if(run.ground.length&&!action.confirm)fail('还有地面奖励没带走。');
      if(spend(run,EXTRACT_SECONDS)){settle(state,true,'安全撤离');return state;}break;
    }
    default:fail('未知操作。');
  }
